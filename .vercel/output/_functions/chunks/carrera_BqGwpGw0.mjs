...
import { $ as AstroError, U as RenderUndefinedEntryError, Y as UnknownContentCollectionError } from "./errors-data_B1kPcZIy.mjs";
import { c as isRemotePath, d as removeBase, u as prependForwardSlash } from "./path_DSAdOJJi.mjs";
import { S as createHeadAndContent, f as renderTemplate, g as maybeRenderHead, h as generateCspDigest, n as renderScriptElement, o as renderComponent, r as renderUniqueStylesheet, t as spreadAttributes, v as addAttribute, w as unescapeHTML } from "./server_DMWtbmF5.mjs";
import { r as VALID_INPUT_FORMATS } from "./consts_Bx4_lkUX.mjs";
import { c as SITE, i as getCarreraConfig, n as $$Layout, r as $$Icon } from "./formato_BK0EqfC1.mjs";
...
var ImmutableDataStore = class ImmutableDataStore {
	...
	*/
	static async fromModule() {
		try {
			const data = await import("./_astro_data-layer-content_BnATZNFF.mjs");
...
async function updateImageReferencesInBody(html, fileName) {
	...
	const { getImage } = await import("./_virtual_astro_get-image_BqPXdLTz.mjs");
...
//#endregion
//#region src/pages/carrera.astro
var carrera_exports = /* @__PURE__ */ __exportAll({
	default: () => $$Carrera,
	file: () => $$file,
	prerender: () => false,
	revalidate: () => 60,
...
var $$Carrera = createComponent(async ($$result, $$props, $$slots) => {
	const carrera = await getCarreraConfig();
	const entradas = await getCollection("carrera", ({ data }) => data.orden);
	const renderizados = await Promise.all(entradas.map(async (entrada) => ({
		...entrada,
		html: (await renderEntry(entrada)).Content
	})));
	...
	})}${maybeRenderHead($$result)}<section class="bg-nocturno pb-16"><div class="mx-auto max-w-6xl px-4 sm:px-6">${carrera ? renderTemplate`${renderComponent($$result, "CarreraBanner", $$CarreraBanner, { "carrera": carrera })}` : renderTemplate`<div class="rounded-2xl border border-albero/30 bg-nocturno-700 p-8 text-center text-white/80">La información de la edición vigente se publicará próximamente. Consultas a ${SITE.email}.</div>`}</div></section><section id="ficha" class="bg-white py-20"><div class="mx-auto max-w-6xl px-4 sm:px-6">${renderComponent($$result, "SectionHeading", $$SectionHeading, {
	...
	})}<div class="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">${servicios.map((s) => renderTemplate`<div class="rounded-xl border border-slate-200 bg-hueso p-7"><span class="flex h-12 w-12 items-center justify-center rounded-full bg-nocturno/5 text-nocturno-600">${renderComponent($$result, "Icon", $$Icon, {
		"name": s.icono,
		"class": "h-6 w-6"
	})}</span><h3 class="mt-5 font-display text-lg font-bold text-nocturno">${s.titulo}</h3><p class="mt-2 text-sm leading-relaxed text-tinta/70">${s.texto}</p></div>`)}</div></div></section>${recorrido.length > 0 && renderTemplate`<section id="recorrido" class="scroll-mt-20 bg-hueso py-20"><div class="mx-auto max-w-4xl px-4 sm:px-6">${renderComponent($$result, "SectionHeading", $$SectionHeading, {
		"eyebrow": "Recorrido",
	...
	})}<div class="mt-10 space-y-8">${recorrido.map((entrada) => renderTemplate`<article class="rounded-xl border border-slate-200 bg-white p-8 shadow-sm"><h3 class="font-display text-2xl font-bold text-nocturno">${entrada.data.titulo}</h3><div class="prose prose-nocturno mt-4 max-w-none text-tinta/80">${renderComponent($$result, "entrada.html", entrada.html, {})}</div></article>`)}</div></div></section>`}${reglamentos.length > 0 && renderTemplate`<section id="reglamento" class="scroll-mt-20 bg-white py-20"><div class="mx-auto max-w-4xl px-4 sm:px-6">${renderComponent($$result, "SectionHeading", $$SectionHeading, {
		"eyebrow": "Reglamento",
		"title": "Bases de la prueba",
		"description": "Normativa de la 10K y de la 5K de iniciación de la edición vigente."
	})}<div class="mt-10 grid gap-8 md:grid-cols-2">${reglamentos.map((entrada) => renderTemplate`<article class="rounded-xl border border-slate-200 bg-hueso p-8"><span class="inline-flex rounded-full bg-nocturno px-3 py-1 text-xs font-bold uppercase tracking-wider text-white">${entrada.data.categoria === "10k" ? "10K" : "5K de iniciación"}</span><h3 class="mt-4 font-display text-2xl font-bold text-nocturno">${entrada.data.titulo}</h3><div class="prose prose-sm prose-nocturno mt-4 max-w-none text-tinta/80">${renderComponent($$result, "entrada.html", entrada.html, {})}</div></article>`)}</div></div></section>`}<section class="bg-nocturno py-16 text-white"><div class="mx-auto max-w-3xl px-4 text-center sm:px-6"><h2 class="font-display text-3xl font-bold">¿Te lo vas a perder?</h2><p class="mt-3 text-white/80">Vive una noche de running bajo las estrellas de San Clemente. Plaza Mayor, 22:00 h.</p><div class="mt-7 flex flex-wrap justify-center gap-3">${carrera?.enlace_inscripcion && renderTemplate`<a${addAttribute(carrera.enlace_inscripcion, "href")} target="_blank" rel="noreferrer noopener" class="inline-flex items-center gap-2 rounded-md bg-albero px-6 py-3 text-sm font-bold text-nocturno transition hover:bg-albero-100">${carrera.texto_cta ?? "Inscríbete"}${renderComponent($$result, "Icon", $$Icon, {
		"name": "external",
		"class": "h-4 w-4"
	})}</a>`}${carrera?.enlace_clasificaciones && renderTemplate`<a${addAttribute(carrera.enlace_clasificaciones, "href")} target="_blank" rel="noreferrer noopener" class="inline-flex items-center gap-2 rounded-md border border-white/30 px-6 py-3 text-sm font-bold text-white transition hover:border-albero hover:text-albero">Ver clasificaciones${renderComponent($$result, "Icon", $$Icon, {
		"name": "external",
		"class": "h-4 w-4"
	})}</a>`}</div></div></section>` })}`;
}, "C:/Users/Anibal/Desktop/club-atletismo-san-clemente/src/pages/carrera.astro", void 0);
var $$file = "C:/Users/Anibal/Desktop/club-atletismo-san-clemente/src/pages/carrera.astro";
var $$url = "/carrera";
//#endregion
//#region \0virtual:astro:page:src/pages/carrera@_@astro
var page = () => carrera_exports;
//#endregion
export { page };
